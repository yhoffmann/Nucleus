...
void HotspotNucleus::sample_hotspot_weights_fixed_avg() {
  ...
  for (uint n = 0; n < m_atomic_num; n++) {
    ...
    for (uint h = 0; h < m_num_hotspots_total; h++)
      m_hotspot_weights[n * m_num_hotspots_per_nucleon + h] /= avg;
  }
}

void HotspotNucleus::reset_hotspot_weights() {
  for (uint h = 0; h < m_num_hotspots_total; h++) m_hotspot_weights[h] = 1.0;
}

void HotspotNucleus::reset_weights() {
  reset_nucleon_weights();
  reset_hotspot_weights();
}
...
HotspotNucleus::HotspotNucleus(const HotspotNucleus& other)
  ...
  std::copy(other.m_hotspot_weights,
            other.m_hotspot_weights + m_num_hotspots_total, m_hotspot_weights);
}
