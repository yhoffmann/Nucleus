...
double HotspotNucleus::get_hotspot_thickness(double x, double y) const {
  ...
  return thickness / double(m_num_hotspots_per_nucleon * M_PI) *
         inverse_r_sqr_divisor;
}
...
const HotspotPos* HotspotNucleus::get_hotspot_pos(uint nucleon_num,
                                                  uint hotspot_num) const {
  return &m_hotspot_pos[nucleon_num * m_num_hotspots_per_nucleon + hotspot_num];
}

const HotspotPos* HotspotNucleus::get_hotspot_pos(
    uint hotspot_num_absolute) const {
...
void HotspotNucleus::sample_hotspot_weights() {
  ...
  } else  // nucleon weights set
  {
    for (uint n = 0; n < m_atomic_num; n++)
      for (uint h = 0; h < m_num_hotspots_per_nucleon; h++)
        m_hotspot_weights[n * m_num_hotspots_per_nucleon + h] =
            m_lognorm_rand() * get_nucleon_weight(n);
  }
}

void HotspotNucleus::sample_hotspot_weights_fixed_avg() {
  prepare_hotspot_weights();
  ...
  if (nullptr == m_nucleon_weights) {
    double avg = 0.0;
    for (uint h = 0; h < m_num_hotspots_total; h++)
      avg += get_hotspot_weight(h);

    avg /= double(m_num_hotspots_per_nucleon);

    for (uint h = 0; h < m_num_hotspots_total; h++) m_hotspot_weights[h] /= avg;
  } else  // nucleon weights set
  {
    double num_double = double(get_num_hotspots_per_nucleon());
    for (uint n = 0; n < m_atomic_num; n++) {
      double avg = 0.0;
      for (uint h = 0; h < m_num_hotspots_per_nucleon; h++)
        avg += get_hotspot_weight(n, h);

      double avg_inverse_times_nucleon_weight =
          num_double / avg * get_nucleon_weight(n);

      for (uint h = 0; h < m_num_hotspots_per_nucleon; h++)
        m_hotspot_weights[n * m_num_hotspots_per_nucleon + h] *=
            avg_inverse_times_nucleon_weight;
    }
  }
}

double HotspotNucleus::get_hotspot_weight(uint nucleon_num,
                                          uint hotspot_num) const {
  return m_hotspot_weights[nucleon_num * m_num_hotspots_per_nucleon +
                           hotspot_num];
}
...
HotspotNucleus::HotspotNucleus(uint seed, uint atomic_num,
                               uint num_hotspots_per_nucleon,
                               double nucleon_size, double hotspot_size,
                               SamplingDistribution sampling_distribution)
...
HotspotNucleus::HotspotNucleus(const HotspotNucleus& other)
  ...
  prepare_hotspot_pos();
  std::copy(other.m_hotspot_pos, other.m_hotspot_pos + m_num_hotspots_total,
            m_hotspot_pos);

  if (nullptr != other.m_hotspot_weights) {
    safe_delete_hotspot_weights();
    prepare_hotspot_weights();
    std::copy(other.m_hotspot_weights,
              other.m_hotspot_weights + m_num_hotspots_total,
              m_hotspot_weights);
  }
}
...
bool HotspotNucleus::fits_hotspot_distribution(double r_sqr) {
  return m_rand() < NormedSamplingDistributions::gaussian(
                        r_sqr, m_nucleon_size * m_nucleon_size);
}
