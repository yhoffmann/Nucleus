...
void Nucleus::sample_nucleon_weights_fixed_avg() {
  ...
  double avg = 0.0;
  for (uint n = 0; n < m_atomic_num; n++) avg += get_nucleon_weight(n);
...
void Nucleus::export_nucleon_positions(double impact_param_x,
                                       double impact_param_y,
  ...
  filestream << "# " << m_mean_bulk_radius << " " << impact_param_x << " "
             << impact_param_y << std::endl;

  for (uint n = 0; n < m_atomic_num; n++) {
    filestream << m_nucleon_pos[n].x << " " << m_nucleon_pos[n].y << " "
               << m_nucleon_pos[n].z << " " << impact_param_x << " "
               << impact_param_y << " " << m_nucleon_size << std::endl;
  }
...
uint Nucleus::get_atomic_num() const { return m_atomic_num; }
...
#ifndef _QUIET
    std::cerr
        << "Accessing nucleon outside of range. Truncating to last nucleon..."
        << std::endl;
#endif
...
double Nucleus::get_mean_bulk_radius() const { return m_mean_bulk_radius; }

double Nucleus::get_mean_surface_diffusiveness() const {
  return m_mean_surface_diffusiveness;
}

double Nucleus::get_nucleon_size() const { return m_nucleon_size; }

void Nucleus::set_nucleon_size(double nucleon_size) {
  m_nucleon_size = nucleon_size;
}

void Nucleus::seed(uint seed) { prepare_rng(seed); }

void Nucleus::seed(const std::mt19937& rng) { prepare_rng(rng); }

Nucleus::Nucleus(uint seed, uint atomic_num, double nucleon_size,
...
void Nucleus::set_mean_bulk_radius() {
  m_mean_bulk_radius = NucleusConstants::fmToGeVm1 *
                       NuclearParameters::get(m_atomic_num).mean_bulk_radius;
}

void Nucleus::set_mean_surface_diffusiveness() {
  m_mean_surface_diffusiveness =
      NucleusConstants::fmToGeVm1 *
      NuclearParameters::get(m_atomic_num).mean_surface_diffusiveness;
}
...
void Nucleus::set_sampling_range() {
  switch (m_sampling_distribution) {
    case SamplingDistribution::WoodsSaxon:
      m_sampling_range =
          m_mean_bulk_radius + 10.0 * m_mean_surface_diffusiveness;
      break;
...
void Nucleus::sample_single_pos(NucleonPos* nucleon_pos) {
  while (true) {
    ...
    double r_sqr = sampled_pos.x * sampled_pos.x +
                   sampled_pos.y * sampled_pos.y +
...
bool Nucleus::fits_nucleon_distribution(double r_sqr) {
  switch (m_sampling_distribution) {
    case SamplingDistribution::WoodsSaxon:
      return m_rand() < NormedSamplingDistributions::woods_saxon(
                            std::sqrt(r_sqr), m_mean_bulk_radius,
                            m_mean_surface_diffusiveness);
